...
const MovieCard = ({movie, onLiked, favorites}) =>{
        ...
        <div className="movie">
            <div className="btn-fav">
                <div className="con-like">
                    <input title="like" type="checkbox" onChange={() => onLiked(movie.imdbID)} checked={favorites.filter((fav) => (fav.movID === movie.imdbID)).length > 0} className="like"/>
            ...
            <div className="more-info">
                <button>
                    <Link to={`/Info?id=${movie.imdbID}`}>
                        More Info
                    </Link>
                </button>
            </div>
            <img className="poster" loading="lazy" draggable='false' src={movie.Poster !== 'N/A' ? movie.Poster : 'https://via.placeholder.com/800x800.png/333&text=Image'} alt={movie.Title} />
            <div className="details">
                <span>{movie.Type}</span>
                <p>{movie.Year !== null ? movie.Year : 'Null'}</p>
                <h3>{movie.Title}</h3>
            </div>
        </div>
    )
